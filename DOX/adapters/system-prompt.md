...
DOX activates when editing files inside a project that has AGENTS.md files. It does not run on conversational, analysis-only, or single-turn tasks with no file edits.
...
- DOX does not score entropy or emit signal weights — it surfaces the constraint once, before the edit.
...
- No child doc may weaken DOX itself.
...
Request
  ...
  → DOX Phase 1 (load contracts, emit contract signal if constraint applies)
  → FUSE + WARD (wrap execution — per action)
  → Edit (artifact produced)
  → FLOW (operational efficiency evaluation, if trigger fires)
  → DOX Phase 2 (closeout — update docs if needed)
